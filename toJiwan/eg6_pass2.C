...
//  gStyle->SetTextSize(0.9);
 ...
 gStyle->SetPaperSize(20,24); 
  ...
  TFile *skim_file = new  TFile("test_all_new_61*_runs.root", "Recreate");
...
/////////////////// find the helicity signals for all the electrons //////////////////
 ...
 //3H
  ...
  for(int i = 0; i<7; i++)
  {
      ...
      h_ch2cc_nphe[i] = new TH2D(Form("h_ch2cc_nphe[%u]",i),
           ...
           h_ch2cc_nphe[i]->SetYTitle("cc_chi2 [rad]");
      }
 ...
 TH1D *h_prot_DeltaBeta =new TH1D("h_prot_DeltaBeta","",140,-0.1,0.1);
  ...
  TH1D *h_z_e_l = new TH1D("h_z_e_l","L electron z_{v}", 300, -51, -46);
        h_z_e_l->SetXTitle("z_{vertex} [cm]");
        h_z_e_l->SetLineWidth(2);

  TH1D *h_z_e_r = new TH1D("h_z_e_r","R electron z_{v}", 300, -51, -46);
        h_z_e_r->SetXTitle("z_{vertex} [cm]");
        h_z_e_r->SetLineWidth(2);
  ...
  int initial_number_of_e1 = 0;
  int initial_number_of_e2 = 0;
  int initial_number_of_e3 = 0;
  int initial_number_of_e4 = 0;
  int initial_number_of_e5 = 0;
  int k_entry = 0;
...
//////////////////// loop over the events ///////////////////////////////////////////////
 ...
 c1->cd();
      ...
      l->DrawLine(-80, 0,-80, h_rtpc_z_r->GetMaximum());
      l->DrawLine( 80, 0, 80, h_rtpc_z_r->GetMaximum());
...
//  h_rtpc_sdist_l->Scale(scale_factor);
      ...
      l->DrawLine(-3,0,-3, h_rtpc_sdist_r->GetMaximum());
...
//  h_rtpc_npd_l->Scale(scale_factor);
      ...
      l->DrawLine(-2,0,-2, h_rtpc_edist_r->GetMaximum());
      l->DrawLine(3,0,3, h_rtpc_edist_r->GetMaximum());
...
//  h_rtpc_X2_l->Scale(scale_factor); 
      ...
      l->DrawLine(3.0, 0,3.0, h_rtpc_X2_r->GetMaximum());
...
//  h_rtpc_z_l->Scale(scale_factor);
    ...
    h_z_e->Draw(); 
      c1->Print("fig/el_vz.png");     
      c1->Print("fig/el_vz.C");     

    h_z_e_r->Draw(); 
      c1->Print("fig/el_r_rtpc_r.png");     
      c1->Print("fig/el_r_rtpc_r.C");     

    h_z_e_l->Draw(); 
      c1->Print("fig/el_r_rtpc_l.png");     
      c1->Print("fig/el_r_rtpc_l.C");     
...
//    l->DrawLine(20,0,20,h_e_nphe_1[i]->GetMaximum());
 ...
 //////////////////////// electron selection in EC////////////////////////////////////////////
   ...
   for(int i= 0; i< TMath::Min(25, gpart); i++)
    {
     initial_number_of_e1++;
       bool flag = false;
    if( p[i]>0.8 && p[i]<6.0 && id[i] ==11 && stat[i]>0 && 
  ...
  if( flag == true)// && -77.<vz[i] && vz[i]<-50.0)
    {
     ...
     initial_number_of_e4++;
             ...
             if (nphe[cc[i]-1] >20 )
              {
     initial_number_of_e5++;
               Q2[n_e_ec] = 4*p[i]*Eb*pow(sin(ec_theta*TORAD/2), 2);
               //cout<< 4*p[i]*Eb<< "     "<<pow(sin(ec_theta*TORAD/2), 2)<<"   "<<Q2[n_e_ec]<<endl;
               nu[n_e_ec] = (Eb - p[i]);
               ...
               if(90.0<El_Phi[n_e_ec] && El_Phi[n_e_ec] <270.0) h_z_e_r->Fill( vz[n_e_ec] );
                else if(90.0>El_Phi[n_e_ec] || El_Phi[n_e_ec] >270.0) h_z_e_l->Fill( vz[n_e_ec] ); 
       ...
       if(IC_theta_En_Cut(etc[j], Phot4VectIC_before[j].Theta()*TODEG) && 
           ...
           etc[j]>0.3 ) 
        {
		         ...
		         Ph_IC_Phi[n_phot_IC]=Phot4VectIC[n_phot_IC].Phi()*TODEG;
                     ...
                     h_photon_phi_theta_2->Fill(Ph_IC_Theta[n_phot_IC], Ph_IC_Phi[n_phot_IC]);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for(int i=0; i<TMath::Min(30, gcpart); i++)
   {
    float Delta_z= ((vz[id_e_ec[0]]+64)*10 - z[i] );   
   ...
   if (abs(Delta_z)<50)
    {
      ...
      if(p_tot[i]<1000.0  //&& -100.0<z[i] && z[i]<100.0  
            && x2[i]<3.5  &&
       ...
       for(int i=0; i<TMath::Min(30, gcpart); i++)
        {
          ...
          if( AcceptRTPC(vz[id_e_ec[0]], theta[i], phi[i]) &&
             ...
             x2[i]<3.0  &&
             r_0[i]>0. && npd_track[i]>3  && abs(sdist[i]) < 2.0 &&
             -1.0<edist[i] && edist[i]<5.0 //&& 
             //abs(Delta_z)<20.0
             )
             {
               if(90.0<phi[i]*TODEG && phi[i]*TODEG <270.0){
                  h_dedx_p_Coh_r->Fill(p_tot[i], dedx[i]);
               }
                else if(90.0>phi[i]*TODEG || phi[i]*TODEG >270.0){
                   h_dedx_p_Coh_l->Fill(p_tot[i], dedx[i]);
                }
              }
          }
       ...
       for(int i = 0; i < TMath::Min(30, rtpc_npart); i++)
        {
          ...
          if(AcceptRTPC(vz[id_e_ec[0]], theta[rtpc_gcpb[i]], phi[rtpc_gcpb[i]])  &&
             ...
             x2[rtpc_gcpb[i]]<3.0   &&  r_0[rtpc_gcpb[i]]>0.0  &&
...
cout<<"  initial_number_of_e1 = "<<initial_number_of_e1<<endl;
cout<<"  initial_number_of_e2 = "<<initial_number_of_e2<<endl;
cout<<"  initial_number_of_e3 = "<<initial_number_of_e3<<endl;
cout<<"  initial_number_of_e4 = "<<initial_number_of_e4<<endl;
cout<<"  initial_number_of_e5 = "<<initial_number_of_e5<<endl;
l->SetLineColor(2);
...
bool AcceptRTPC( float vz, float theta, float phi)
{
    ...
    float phi2=GEO.GetPhi(phi,0)*R2D;
